# SliceMatic Chat Architecture â€” the AI ordering flow, and how to build one like it
...
>
> Verified live 2026-07-03: full order in 8 turns, ~1.3â€“2.1s per turn after warmup.
...
sequenceDiagram
    ...
    alt blocked (injection / abuse / off-topic)
        ...
        alt LLM replies with text
            ...
            A-->>U: INJECTED "[BILL]{json}[/BILL] ... [PAYMENT_OPTIONS]"<br/>(bypasses the LLM â€” deterministic, zero extra latency)
        end
        Note over A,L: later, once payment is settledâ€¦
        L->>T: confirm_and_save_order (only NOW in the tool list)
        T->>D: create_order â€” Supabase ONLY, DB-generated SM-YYYYMMDD-NNNN
        T-->>A: {"ok": true, "order_no": ...}
        A-->>U: INJECTED localized "âœ… Order confirmed! ..."
    end
```
...
2. **Gate tools by stage, don't beg in the prompt.** The save tool isn't *discouraged*
   early â€” it isn't *in the tool list* until it's legal. Wrong-time calls become impossible.
3. **Inject deterministic outputs.** When a tool's result is already the final answer
   (a bill, a confirmation), return it directly â€” skipping the second LLM pass removes
...
|---|----------|--------------|--------------|-------|------|
| 1 | "hi, I'd like to order a pizza" | building | â€” | greeting + `[TILES:PIZZA: â€¦12 live pizzas]` | ~1.6s |
| 2 | "Margherita" | building | â€” | `[TILES:BASE: â€¦6 bases]` | 1.7s |
| 3 | "Cheese Burst" | building | â€” | `[TILES:TOPPING: â€¦10 toppings]` | 1.7s |
| 4 | "Extra Cheese and Jalapenos" | building | â€” | asks quantity (1â€“10) | 1.3s |
| 5 | "2 please, and that's all" | building | `get_customer_profile` + `calculate_order_price` (one round) | **injected** `[BILL]{json}[/BILL] â€¦ [PAYMENT_OPTIONS]` | **1.3s** |
| 6 | "UPI" (payment button) | payment | â€” | `[UPI_QR]` (frontend renders QR + "I have paid") | 2.1s |
| 7 | "I have paid via UPI" | payment | `confirm_and_save_order` â†’ Supabase | **injected** "âœ… Order confirmed! SM-20260702-0004 â€” â‚¹1500.96 via UPI" | **1.7s** |

Pricing was exact core math: unit â‚¹636 (base 229 + pizza 299 + toppings 69 + 39), Ã—2 =
â‚¹1272 subtotal, no discount (qty < 5), GST â‚¹228.96, **total â‚¹1500.96**. The Supabase row
carried `source: "chat"`, the session id, the hardcoded profile's `user_id`, and the full
line breakdown in `items` jsonb â€” and `orders_log.txt` was untouched.

---

## 3. Stages: a state machine derived, not stored

The flow is linear (build â†’ pay â†’ done), but customers aren't â€” they change their minds,
...
stateDiagram-v2
    ...
    payment --> payment: customer edits order â†’<br/>reprice, bill re-injected
    ordered --> payment: orders again â†’<br/>calculate resets confirmed
    building --> escalated: escalate_to_human
    payment --> escalated: escalate_to_human
    ordered --> escalated: escalate_to_human
```

Derivation (`agent._stage`, ~8 lines â€” this is the whole "framework"):
...
|---|---|---|
| **building** | `get_customer_profile`, `calculate_order_price`, `escalate_to_human` | Pizza â†’ base â†’ 1â€“3 toppings â†’ quantity; one question at a time; offer `[TILES:â€¦]`; ask "another pizza?"; when done, call profile + calculate **together**; fill every slot a multi-detail message gives. |
| **payment** | building set **+ `confirm_and_save_order`** | Bill/buttons already on screen â€” don't repeat amounts. UPI â†’ `[UPI_QR]`, Card â†’ `[CARD_FORM]`, Cash/COD â†’ no tag. The moment payment settles, save with the SAME items â€” don't re-ask. Order edits â†’ reprice. |
| **ordered** | building set (save is gone again) | Saved â€” do NOT save again. Answer follow-ups; a new order flows building â†’ payment again. |
...
the system prompt every turn â€” a lookup tool would just add a wasted LLM round trip) and
...
| `calculate_order_price` | order complete / after any edit | Validates each line (`core.validation`), fuses 1â€“3 toppings into one `MenuItem` (`_combined_topping`), prices via `core.pricing.compute_bill`; resets `confirmed` | `{"ok": true, lines, cart}` JSON or error text |
| `confirm_and_save_order` | payment settled (**gated**) | Output guardrail â†’ `db.orders.create_order` (**Supabase only**, one row/order, `items` jsonb, DB `order_no`, `user_id` stamped); failure surfaced, order NOT placed | `{"ok": true, order_no, total, payment_mode}` or error text |
...
**Persistence split (decided):** Stage 3 chat/voice orders use Supabase as the source of truth
...
current. Structure (order matters â€” start and end get the most model attention):

```
1. IDENTITY + LANGUAGE   one persona line; "Respond ONLY in {English|Hindi}"
2. HARD RULES            never compute money Â· only menu items Â· 1 line = pizza+base+1-3 toppings+qty
                         profile for name/phone Â· one tool at a time (exception: profile+calculate)
3. OUTPUT TAGS           every UI tag defined ONCE: [TILES:CAT: â€¦] / [UPI_QR] / [CARD_FORM]
                         "the system shows [BILL]/[PAYMENT_OPTIONS] itself â€” never write them"
4. FEW-SHOT EXAMPLES     3 mini-exchanges built from the LIVE menu (grader swaps files â†’
                         names must never be hardcoded); models copy examples â‰« obey prose
5. MENU (fenced)         --- MENU --- {live items+IDs+prices} --- END MENU ---
6. CURRENT STEP          the one stage instruction â€” the model juggles one step, not six
```

Anti-patterns this replaced (all were live bugs): the same rule stated twice in different
words Â· format rules filed under a "tools" header Â· "call get_menu" *and* an embedded menu Â·
two different triggers for saving Â· showing the malformed tag as a "don't" example Â·
...
`ai/session.py` â€” an in-memory dataclass per `session_id` (frontend generates the id):
`history` (OpenAI-shaped messages incl. tool calls), `language` (re-detected every turn â€”
Devanagari/Hinglish â†’ `hi`), `channel` (chat|voice), `name/phone` (primed by the profile
tool), `pricing`, `confirmed`, `human_escalated`, `voice_started_at` (3-min cap).

- A **per-session `threading.Lock`** serialises turns â€” no interleaved tool loops.
...
- Stage is **not** a field â€” it is derived (see Â§3). One less thing to get stale.
...
## 7. Guardrails â€” asymmetric by design

**Input (before the LLM), fails OPEN:** cheap heuristics first (injection regexes, abuse
words, â‰¤3-word messages and food-words pass free), then a cheap-LLM classifier
(`gpt-4o-mini`, 4 max tokens) only when unsure. A classifier outage lets traffic through â€”
...
(plum `#A16E83` â€” the palette has no red) â€” the visible cue that the guardrail, not the
model, answered.
...
qty 1â€“10. Any failure â†’ error text back to the LLM to re-prompt; **nothing is ever
...
## 8. Latency engineering (10s â†’ ~1.5s)

Measured levers, in impact order:

| Lever | Where | Why it worked |
|---|---|---|
| Disable hidden reasoning | `_complete`: `extra_body={"reasoning": {"enabled": False}}` | `gemini-2.5-flash` *thinks* by default â€” seconds per call for a form-filling task that needs none |
...
| `[timing]` logs at every seam | STT / TTS / LLM / agent | you cannot fix what you attribute wrongly â€” our "server is slow" turned out to be ~2s of *client-side* HTTP overhead in one measurement |
...
## 9. Guidelines â€” applying this to a similar use case
...
2. **Find the commit point; gate it mechanically.** One tool is irreversible (save/pay/
   book). Expose it only when code-checked preconditions hold â€” `tools_for(state)` is a
...
5. **Inject deterministic outputs.** If a tool result is final (receipt, confirmation,
   OTP screen), return it verbatim â€” don't pay an LLM pass to (mis)copy numbers.
...
8. **Idempotency at the gate.** After a save, the commit tool disappears (`confirmed`)
   until a new priced state exists â€” double "I have paid" cannot double-write.
9. **Warm the real path at boot** â€” full prompt + tools, not a toy ping â€” on a background
...
11. **Skip agent frameworks until proven otherwise.** LangChain/LangGraph call the same
    ...
    The three things that mattered â€” gating, injection, staged prompt â€” are ~60 lines of
    plain Python on the OpenAI SDK, and observability (Langfuse drop-in) stayed intact.
12. **Multi-model fallback at the app level.** PRIMARY â†’ FALLBACK1 â†’ FALLBACK2 per call;
...
| Wrong-tool-call prevention | Stage-gated `tools_for(session)` | Prompt-only rules ("call ONLY afterâ€¦") â€” measured unreliable |
| Bill / confirmation rendering | Backend injection (`[BILL]` JSON, localized confirm text) | LLM reads tool result back â€” slower, mangles numbers |
...
| Chat/voice persistence | Supabase only (`create_order`: 1 row/order, `items` jsonb, DB `order_no` `SM-YYYYMMDD-NNNN`, `user_id`) â€” failure surfaced | `.txt` + best-effort mirror (`.txt` is historical compatibility output) |
| Orders listing | `GET /api/orders?phone=` (interim; `user_id` after auth) | â€” |
...
*Related: `CLAUDE.md` (Stage-3 status + conventions) Â· `docs/STAGE3_PLAN.md` Â·
`docs/API.md` Â· `ai/agent.py` / `ai/tools.py` / `ai/guardrails.py` / `ai/session.py`.*

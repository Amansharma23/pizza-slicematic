...
/**
 * REST fallback call machine — the original batch implementation (record on
 * client-side VAD silence -> upload -> STT -> agent -> TTS -> download -> play).
 * Preserved verbatim as a safety net: set NEXT_PUBLIC_VOICE_MODE=rest to use
 * this instead of the realtime WebSocket pipeline in use-voice.ts. See
 * reference/VOICE_PIPELINE_ARCHITECTURE.md for why the realtime version
 * replaced this as the default (batch pipelines have several seconds of dead
 * air per turn, and client-side VAD stalls when the tab is backgrounded).
 */
...
export function useRestVoiceCall() {

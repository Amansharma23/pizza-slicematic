...
import { Bike, ChefHat, KeyRound, Loader2, MonitorSmartphone, UserPlus } from "lucide-react";
...
/**
 * Admin home = employee management: create staff / kitchen_staff / delivery
 * accounts (the DB assigns the EMP-xxx id; the admin shares id + PIN with the
 * employee), reset PINs, deactivate/reactivate. More admin features (menu,
 * analytics, escalations) land in later milestones.
 */

const ROLES: { id: Role; label: string; icon: typeof Bike }[] = [
  { id: "staff", label: "Staff (kiosk)", icon: MonitorSmartphone },
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (res.ok && res.employees) {
        ...
        setListError(first ?? "Couldn't load employees.");
      }
    } catch (err) {
      setListError(
        err instanceof Error ? err.message : "Couldn't load employees."
      );
    } finally {
      setLoading(false);
    }
    ...
    <div className="mx-auto w-full max-w-5xl space-y-6 px-6 py-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Create kiosk, kitchen and delivery accounts — share the generated
          employee ID and PIN with them directly.
        </p>
      </div>

      <div className="grid gap-6 lg:grid-cols-[360px_1fr]">
        <CreateEmployeeCard onCreated={load} />
        <EmployeeTable
          employees={employees}
          loading={loading}
          error={listError}

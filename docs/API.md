...
Base URL (local): `http://localhost:7860`. A Postman collection is in
`postman/SliceMatic.postman_collection.json`.

Two surfaces share the same `core/` brain (menu, validation, pricing, persistence):
- **`/api/*`** — deterministic REST (also used by the Gradio app).
- **`/chat`, `/voice/*`** — conversational AI (LLM orchestrates `core/` via tools).
...
```
On failure, `ok=false` and `errors` maps field → message.
...
```
Validation/menu errors → `{ "ok": false, "errors": { ... } }`.
...
Get/set the discount rate. POST body: `{ "discount_rate": 0.10 }` → `{ "ok": true, "discount_rate": 0.10 }`.
...
```
- `session_id` optional — omit to start a new conversation (one is generated and returned).
- Flow: input guardrail → LLM agent (tools → `core/`) → reply. Transcript saved to `messages`.
...
```
- `blocked: true` → message was redirected by the input guardrail (no LLM call); `reply` is the redirect.
- `escalated: true` → the agent handed off to a human (an `escalations` row was created).
...
No telephony — the browser captures mic audio and plays the response.
...
- **Resilience**: Supabase writes are best-effort with transient-error retries; a DB outage
  never blocks an order (the `.txt` log is primary).
